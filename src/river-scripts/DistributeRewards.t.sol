...
contract DistributeRewardsTestSetupBase is LlamaTestSetup {
  ...
  INodeOperator internal constant NODE_OPERATOR = INodeOperator(REGISTRY_DIAMOND);
  IMainnetDelegation internal constant MAINNET_DELEGATION = IMainnetDelegation(REGISTRY_DIAMOND);
...
contract DistributeRewardsBase is DistributeRewardsTestSetupBase {
  function test_Base_DistributeRewards() public {
    ...
    uint256 frameworkCommissionRate = NODE_OPERATOR.getCommissionRate(FRAMEWORK);
    ...
    address frameworkClaimer = NODE_OPERATOR.getClaimAddressForOperator(FRAMEWORK);
    ...
    assertEq(frameworkClaimerBalancePost, frameworkClaimerBalancePre + frameworkClaimableAmtPost);
    ...
    assertEq(hanedaClaimerBalancePost, hanedaClaimerBalancePre + hanedaClaimableAmtPost);
    ...
    assertEq(ohareClaimerBalancePost, ohareClaimerBalancePre + ohareClaimableAmtPost);
    ...
    assertEq(hntlabsClaimerBalancePost, hntlabsClaimerBalancePre + hntlabsClaimableAmtPost);
  }
    ...
    */
    ...
    for (uint256 i = 0; i < baseOperators.length; i++) {
      ...
      for (uint256 j = 0; j < delegations.length; j++) {
        ...
        uint256 claimableAmt = REWARDS_DISTRIBUTION.getClaimableAmountForDelegator(delegator);
        console.log("delegator claimable amount post: ", claimableAmt, " for operator: ", baseOperators[i].name);
        console.log("delegator: ", delegator, " for operator: ", baseOperators[i].name);
        console.log("claimer for delegator: ", claimer, " for operator: ", baseOperators[i].name);
        uint256 claimerBalancePre = RVR_TOKEN.balanceOf(claimer);
        vm.prank(claimer);
